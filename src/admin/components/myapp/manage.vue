  ...
  <div class="myapp-manage">
    ...
    >
      ...
      <div slot="actions" slot-scope="actions,record">
        ...
        <a-popover title="分享应用" trigger="click">
          <div slot="content">
            <a-row>
              <a-col>分享给：</a-col>
              <a-col><a-input placeholder="请输入对方手机号" @change="sharePhoneChange"/></a-col>
            </a-row>
            <a-row><a-button @click="shareTo(record)">确认</a-button></a-row>
          </div>
          <a-tag color="#17BC94">分享</a-tag>
        </a-popover>
        <a-tag
...
export default class MyappManage extends Vue {
  ...
  shareTo(record) {
    ...
    let res: any = service.shareApp(userID, params);
    if (res.data.status === 200) {
